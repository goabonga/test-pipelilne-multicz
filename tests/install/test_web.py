...
"""Validate the installed `shomer-web` Debian package — including the
...
def test_console_script_on_path() -> None:
    assert shutil.which("shomer-web"), "shomer-web console-script missing from PATH"
...
def test_module_imports(run, system_python) -> None:
    out = run(
        system_python,
        "-c",
        "import shomer_web; print(shomer_web.__version__)",
    )
...
def test_module_version_matches_bumped_value(run, system_python, versions) -> None:
    ...
    assert installed == expected, (
        f"installed shomer_web.__version__ = {installed!r} but VERSION says {expected!r}"
    )
...
def test_fastapi_app_object_loads(run, system_python) -> None:
    out = run(
        system_python,
        "-c",
        "from shomer_web.app import app; print(app.title)",
    )
    assert out.stdout.strip(), "shomer_web.app.app resolved to a falsy object"
